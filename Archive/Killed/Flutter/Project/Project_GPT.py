...
import openai  # GPT API를 위한 라이브러리
import os
import socket
...
class APIKeyDialog(QDialog):
    ...
    def initUI(self):
        ...
        self.apply_button.clicked.connect(self.applyAPIKey)
        vbox.addWidget(self.apply_button)

        self.setLayout(vbox)
        self.setWindowTitle('OpenAI API Key')

    def applyAPIKey(self):
...
class DataAnalysisApp(QMainWindow):
    ...
    def runAnalysis(self):
        ...
        if self.df is not None and self.target_column is not None and self.selected_model:
            ...
            def analysis_complete(result):
                ...
                if not self.continue_analysis:
                    return
                
                if self.api_key and self.api_key != "":
                    openai.api_key = self.api_key
                    response = openai.Completion.create(
                        engine="davinci", prompt=result, max_tokens=100
                    )
                    summary_result = response.choices[0].text
                    self.report_text_widget.setPlainText(summary_result)
    ...
    def checkInternetConnection(self):
        hostname = "www.google.com"  # 구글을 통한 연결 테스트
        ...
        if response == 0:
            ...
            msg_box.setText("인터넷 연결이 되어 있지 않습니다.")
            msg_box.setWindowTitle("경고")
            msg_box.setStandardButtons(QMessageBox.Cancel | QMessageBox.Continue)
...
if __name__ == '__main__':
    ...
    # API Key를 입력받는 Dialog를 실행
    api_key_dialog = APIKeyDialog()
    if api_key_dialog.exec_():
        api_key = api_key_dialog.api_key

        # 메인 윈도우 실행
        mainWindow = DataAnalysisApp()
        mainWindow.api_key = api_key  # API 키 전달
        mainWindow.show()
