...
class DataAnalysisApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.getApiKey()  # 앱이 실행될 때 API 키를 얻는 메서드를 호출합니다

    def getApiKey(self):
        api_key, ok = QInputDialog.getText(self, 'API Key', 'API 키를 입력하세요:')
        if ok and api_key:
        ...
        if self.df is not None and self.target_column is not None and self.selected_model:
            ...
            def analysis_complete(result):
                if self.api_key:
                    ...
                    os.environ['_BARD_API_KEY'] = str(self.api_key)
                    response = bardapi.core.Bard().get_answer(result)
                    result = response['choices'][0]['content'][0]
                    response = bardapi.core.Bard().get_answer(str(result))['content']
                    self.report_text_widget.setPlainText(result)
                else:
                    self.result_text_widget.setPlainText(result)  # Statmodels 결과만 표시
